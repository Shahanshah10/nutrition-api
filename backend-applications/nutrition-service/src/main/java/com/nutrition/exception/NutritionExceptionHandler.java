...
public class NutritionExceptionHandler {
    ...
    @ExceptionHandler(value = NutritionServiceException.class)
    public ResponseEntity<ErrorResponse> handleSupplierAlreadyExistsException(NutritionServiceException ex) {
        log.error(HttpStatus.BAD_REQUEST +": "+ ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ErrorResponse.builder().statusCode(HttpStatus.BAD_REQUEST.value())
    ...
    public ResponseEntity<ErrorResponse> handleMethodArgumentNotValidException(MethodArgumentNotValidException ex) {
        List<String> errorMessages = Objects.requireNonNull(ex.getBindingResult()).getFieldErrors()
